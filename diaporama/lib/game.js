...
// public
	...
	Build()
	{
		this.mStage.addChild( this.mAssets.GetGBackground() );

		this.mCurrentSprite = this.mAssets.GetGSprites()[0];
		let current_tweens = this.mAssets.GetTSprites()[0];
		this.mCurrentSprite.x = current_tweens[0].position.x;
		this.mCurrentSprite.y = current_tweens[0].position.y;
		this.mStage.addChild( this.mCurrentSprite );
		...
		this.mGContinue.x = this.mStage.canvas.width / 2;
		this.mGContinue.y = this.mStage.canvas.height / 2;
		this.mStage.addChild( this.mGContinue );
	}
...
// private
	...
	_StartAnimation()
	{
		this.mStage.removeChild( this.mGContinue );

		//---

		let tween = createjs.Tween.get( this.mCurrentSprite, { 'override': true } );
		
		let index = this.mAssets.GetGSprites().indexOf( this.mCurrentSprite );
		let tweens_param = this.mAssets.GetTSprites()[index];
	...
	_StopAnimation()
	{
		this.mStage.removeChild( this.mCurrentSprite );
		
		//---

		let index = this.mAssets.GetGSprites().indexOf( this.mCurrentSprite );
		let next_index = index + 1;
		if( next_index === this.mAssets.GetGSprites().length )
		{
			this._Stop();
			return;
		}

		this.mCurrentSprite = this.mAssets.GetGSprites()[next_index];
